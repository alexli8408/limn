      ...
      items: {
        ...
        properties: {
          ...
          ops: {
            ...
            description:
              "What this group was MEANT to look like, drawn steadily, at most 6. align-left/right/top/bottom snap its members to a shared edge, align-center-x/align-center-y to a shared centre line. distribute-x/distribute-y even out the gaps along one axis. equalize-size gives every member one size. straighten turns a wobbly line into a straight segment. regularize rounds a near-circle to a circle and squares up a near-square. match-style unifies stroke width, roughness and colour. You are not deciding what is safe: every op is checked against each stroke before it is applied and declined where it does not fit, so a curve survives straighten and a deliberate squiggle survives regularize. Say what the thing is meant to be and let the check refuse what it must. Hand-drawn house walls were meant to be straight and square: straighten and regularize. A hand-drawn sun was meant to be a circle: regularize. match-style alone changes almost nothing visible, so it is rarely the whole answer. An empty list says the group is already exactly as intended, which is rare in a hand drawing; use it when the looseness is the point, not when you are unsure.",
          },
        },
